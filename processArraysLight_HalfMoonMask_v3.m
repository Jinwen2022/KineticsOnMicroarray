...
if ~iscell(dirFluo)
    ...
    slide = stitchArrayImages(dirFluo{1},parameters.overlap,parameters.angularDisplacementTile,parameters.rangePositions,16);
    im = preprocessImage(slide,parameters.angularDisplacement,parameters.roi);
end
    [mask,~, ~,HalfMoonSpotsIDs, bkgMask] = arrayGridMask_HalfMoonMask_v3(im,parameters.pxSize,parameters.topLeftCorner,thresholdDNAIndices);
